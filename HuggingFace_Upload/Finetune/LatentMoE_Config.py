...
class MoEBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        topk_probs, topk_idx = torch.topk(gate_probs, self.experts_per_token, dim=-1)
        ...
        load.scatter_add_(0, topk_idx_flat.reshape(-1), torch.ones_like(topk_idx_flat.reshape(-1), dtype=x.dtype))
        balance_loss = 0.5 * (
...
class LatentMoEShimConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.experts_per_token   = experts_per_token
        self.balance_loss_weight = balance_loss_weight
        self.adaptor_config      = adaptor_config or {}

class LatentMoEShim(PreTrainedModel):
    config_class = LatentMoEShimConfig

    def __init__(self, latent_model: LatentMoE, config: LatentMoEShimConfig):
