...
qa_dict = DatasetDict({"train": qa_train, "validation": qa_val})
sft_dict = DatasetDict({"train": sft_train, "validation": sft_val})
...
os.makedirs(os.path.join(out_qa, "validation"), exist_ok=True)
os.makedirs(os.path.join(out_sft, "train"), exist_ok=True)
os.makedirs(os.path.join(out_sft, "validation"), exist_ok=True)

qa_dict["train"].save_to_disk(os.path.join(out_qa, "train"))
qa_dict["validation"].save_to_disk(os.path.join(out_qa, "validation"))
sft_dict["train"].save_to_disk(os.path.join(out_sft, "train"))
sft_dict["validation"].save_to_disk(os.path.join(out_sft, "validation"))

print("QA_stats:", json.dumps({"train":log_tr, "validation":log_va}, ensure_ascii=False, indent=2))
